...
/// </summary>
public class TransitionComponentBase : IComponent, IHandleEvent, IHandleAfterRender
{
    ...
    /// </summary>
    public TransitionComponentBase()
    {
    ...
    /// </summary>
    protected async Task StateHasChanged()
    {
        ...
        if (_hasNeverRendered || ShouldRender() || _renderHandle.IsRenderingOnMetadataUpdate)
        {
            ...
            try
            {
                _jsModule ??= await JsRuntime.InvokeAsync<IJSInProcessObjectReference>("import", "./_content/BlazorViewTransitions/transition.js");
                await _jsModule.InvokeVoidAsync("startTransition", DotNetObjectReference.Create(this));
            }
            catch
            {
                Console.WriteLine("Exception invoking startTransition");
                _hasPendingQueuedRender = false;
                throw;
            }
        }
    ...
    Task IHandleAfterRender.OnAfterRenderAsync()
    {
        ...
        _ = _jsModule!.InvokeVoidAsync("renderFinished");
